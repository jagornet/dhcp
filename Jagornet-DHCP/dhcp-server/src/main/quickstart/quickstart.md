...
- [Jagornet-DHCP Quick Start Guide](#jagornet-dhcp-quick-start-guide)
  ...
  - [Installation and Initial Setup](#installation-and-initial-setup)
    ...
    - [Linux/macOS](#linuxmacos)
      ...
      - [Running as a Daemon (Background Service using YAJSW)](#running-as-a-daemon-background-service-using-yajsw)
    - [Windows](#windows)
      - [Running in Console Mode (Foreground)](#running-in-console-mode-foreground)
      - [Running as a Service (Background Service using YAJSW)](#running-as-a-service-background-service-using-yajsw)
...
*   `yajsw-stable-11.0/` (or similar version): Contains the "Yet Another Java Service Wrapper" (YAJSW) files. YAJSW is used for running the server as a background daemon on Linux/macOS or as a Windows service.
*   `logs/`: This directory will be created automatically when the server runs for the first time. It will contain log files generated by the server (e.g., `JagornetDhcpServer.log`) and by YAJSW (e.g., `wrapper.log`) if used.
...
```bash
chmod +x ./bin/dhcpserver
chmod +x ./bin/yajsw-stable-11.0/bat/*.sh 
# Note: The 'yajsw-stable-11.0' directory name might vary slightly. Adjust if needed.
```

#### Running Directly (Foreground)

This method is useful for testing, debugging, or simple setups. The server will run in your current terminal session.
...
*   **Start the server (with its default configuration file, e.g., `config/dhcpserver.xml`):**
    ```bash
    ./bin/dhcpserver start
    ```
    The server will log to the console and also create/append to `logs/JagornetDhcpServer.log`.
    A PID (Process ID) file named `dhcpserver.pid` will be created in the server home directory.

*   **Start the server with a specific configuration file:**
    ```bash
    ./bin/dhcpserver start -c /path/to/your/custom_dhcpd.conf
    # or relative to server home:
    ./bin/dhcpserver start -c config/alternative_dhcpd.conf
    ```
...
*   **Stop the server:**
    ...
    ./bin/dhcpserver stop
    ```
    This reads the PID from `dhcpserver.pid` and sends a shutdown signal to the process.

*   **Check server status:**
    ```bash
    ./bin/dhcpserver status
    ```
    This checks if the process listed in `dhcpserver.pid` is running.

*   **Restart the server:**
    ```bash
    ./bin/dhcpserver restart
    ```
...
*   **Test configuration file syntax:**
    ...
    # Or with the server's default configuration file (e.g., config/dhcpserver.xml if it exists):
    ./bin/dhcpserver test-configfile
    ```

#### Running as a Daemon (Background Service using YAJSW)

YAJSW (Yet Another Java Service Wrapper) is included to allow the server to run as a proper background daemon. The scripts for YAJSW are located in `$JAGORNET_DHCP_HOME/bin/yajsw-stable-11.0/bat/` (or a similar versioned YAJSW directory). Remember to set execute permissions for these scripts as shown in the "Important: Script Permissions" note above.

1.  **Navigate to the YAJSW scripts directory:**
    ```bash
    cd $JAGORNET_DHCP_HOME/bin/yajsw-stable-11.0/bat/ 
    # Example: cd /opt/Jagornet-DHCP-server-1.2.3/bin/yajsw-stable-11.0/bat/
    # Adjust 'yajsw-stable-11.0' if your version is different.
    ```
    Ensure `JAGORNET_DHCP_HOME` is set correctly in your environment. The `wrapper.conf` file (located in `../conf/` relative to this directory, i.e., `$JAGORNET_DHCP_HOME/bin/yajsw-stable-11.0/conf/wrapper.conf`) is pre-configured to use `JAGORNET_DHCP_HOME`.

2.  **Install the daemon:**
    This usually creates symlinks or copies files to system locations, so `sudo` is often required.
    ```bash
    sudo ./installDaemon.sh
    ```
    This will typically register the service with your system's init system (e.g., systemd). The configuration for the wrapper is in `../conf/wrapper.conf`.

3.  **Start the daemon:**
    *   Using the YAJSW script:
        ```bash
        sudo ./startDaemon.sh
        ```
    *   Or, if successfully integrated with systemd (common on modern Linux):
        ```bash
        sudo systemctl start JagornetDhcpServer # Service name might vary, check installDaemon.sh output
        ```
...
4.  **Stop the daemon:**
    ...
    *   Or, using systemd:
        ```bash
        sudo systemctl stop JagornetDhcpServer
        ```

5.  **Check daemon status:**
    *   Using the YAJSW script:
        ```bash
        sudo ./statusDaemon.sh
        ```
    *   Or, using systemd:
        ```bash
        sudo systemctl status JagornetDhcpServer
        ```
...
Logs for the daemon are typically found in `$JAGORNET_DHCP_HOME/logs/wrapper.log` and `$JAGORNET_DHCP_HOME/logs/JagornetDhcpServer.log`.
...
2.  **Navigate to the server home directory:**
    ...
    ```
3.  **Run the server:**
    The primary script is `bin\JagornetDhcpServer.bat`. It uses similar commands as the Linux shell script.
    *   **Start the server (with its default configuration file, e.g., `config\dhcpserver.xml`):**
        ```cmd
        bin\JagornetDhcpServer.bat start
        ```
    *   **Start with a specific configuration file:**
        ```cmd
        bin\JagornetDhcpServer.bat start -c C:\path\to\your\custom_dhcpd.conf
        ```
    *   **Stop the server (if started via `start` and console window is separate):**
        This might require closing the console window or Ctrl+C in the window where it's running. The `.bat` script's `stop` command is more relevant for the service.
    *   **Show server version:**
        ```cmd
        bin\JagornetDhcpServer.bat version
        ```
    *   **List available network interfaces:**
        ```cmd
        bin\JagornetDhcpServer.bat list-interfaces
        ```
    *   **Test configuration file syntax:**
        ```cmd
        bin\JagornetDhcpServer.bat test-configfile -c config\my_custom_config.xml
        REM Or with the server's default configuration file:
        REM bin\JagornetDhcpServer.bat test-configfile
        ```
    A PID file `dhcpserver.pid` is also created in the server home directory (`%JAGORNET_DHCP_HOME%`).

#### Running as a Service (Background Service using YAJSW)

YAJSW allows Jagornet-DHCP to be installed as a Windows Service.

1.  **Open Command Prompt as Administrator:** Search for `cmd`, right-click, and choose "Run as administrator".
2.  **Navigate to the YAJSW scripts directory:**
    ```cmd
    cd %JAGORNET_DHCP_HOME%\bin\yajsw-stable-11.0\bat\
    REM Example: cd C:\Program Files\Jagornet-DHCP-server-1.2.3\bin\yajsw-stable-11.0\bat\
    REM Adjust 'yajsw-stable-11.0' if your version is different.
    ```
    Ensure `JAGORNET_DHCP_HOME` is set correctly in your system environment variables. The `wrapper.conf` file (located in `..\conf\` relative to this directory, i.e., `%JAGORNET_DHCP_HOME%\bin\yajsw-stable-11.0\conf\wrapper.conf`) is pre-configured to use `JAGORNET_DHCP_HOME`.

3.  **Install the service:**
    ```cmd
    InstallJagornetDhcpServer.bat
    ```
    This will register "Jagornet DHCP Server" (or a similar name) in the Windows Services console. The configuration for the wrapper is in `..\conf\wrapper.conf`.

4.  **Start the service:**
    *   Using the script:
        ```cmd
        StartJagornetDhcpServer.bat
        ```
    *   Or, from the Services console:
        1.  Open Services (search for "services.msc").
        2.  Find "Jagornet DHCP Server" in the list.
        3.  Right-click and select "Start".

5.  **Stop the service:**
    *   Using the script:
        ```cmd
        StopJagornetDhcpServer.bat
        ```
    *   Or, from the Services console: Right-click "Jagornet DHCP Server" and select "Stop".

6.  **Check service status:**
    *   The Services console will show the status (e.g., "Running").
    *   The `QueryStatusJagornetDhcpServer.bat` script might also be available.

7.  **Uninstall the service:**
    ```cmd
    UninstallJagornetDhcpServer.bat
    ```
    This must also be run from an Administrator Command Prompt.

Logs for the service are typically found in `%JAGORNET_DHCP_HOME%\logs\wrapper.log` and `%JAGORNET_DHCP_HOME%\logs\JagornetDhcpServer.log`.
...
*   **Default File Loading:**
    *   When the server starts without a specific configuration file specified (e.g., `./bin/dhcpserver start`), it looks for a default configuration file in the `config/` directory.
...
*   **YAJSW Wrapper Logs (if running as a service/daemon):**
    When running Jagornet-DHCP as a service or daemon using YAJSW (Yet Another Java Service Wrapper), YAJSW itself produces its own logs related to the service management wrapper.
    *   Default YAJSW log file location: `$JAGORNET_DHCP_HOME/logs/wrapper.log`. The path and behavior of these logs can be configured in `$JAGORNET_DHCP_HOME/bin/yajsw-stable-11.0/conf/wrapper.conf` (path to YAJSW may vary slightly).
...
*   **Linux/macOS:**
    *   **Directly (Foreground):**
        ...
        ```bash
        sudo ./bin/dhcpserver start 
        # Use 'sudo' if server needs to listen on privileged ports (like 67, 547)
        # or if configured to use low-numbered ports.
        ```
    *   **As a Daemon (Background Service using YAJSW):**
        Navigate to `$JAGORNET_DHCP_HOME/bin/yajsw-stable-11.0/bat/` (or similar YAJSW versioned dir) and run:
        ```bash
        sudo ./startDaemon.sh
        ```
        Or, if integrated with systemd: `sudo systemctl start JagornetDhcpServer`
*   **Windows:**
    *   **Console Mode (Foreground):**
        ...
        bin\JagornetDhcpServer.bat start
        ```
    *   **As a Service (Background Service using YAJSW):**
        Navigate to `%JAGORNET_DHCP_HOME%\bin\yajsw-stable-11.0\bat\` (or similar) and run (as Administrator):
        ```cmd
        StartJagornetDhcpServer.bat
        ```
        Or, start "Jagornet DHCP Server" from the Windows Services console.
...
*   **Linux/macOS:**
    *   **Directly:**
        ```bash
        sudo ./bin/dhcpserver stop
        ```
    *   **Daemon:**
        ```bash
        sudo ./bin/yajsw-stable-11.0/bat/stopDaemon.sh
        ```
        Or, using systemd: `sudo systemctl stop JagornetDhcpServer`
*   **Windows:**
    *   **Console Mode:** Press `Ctrl+C` in the console window where the server is running.
    *   **Service:**
        ```cmd
        StopJagornetDhcpServer.bat 
        ```
        (run as Administrator from `%JAGORNET_DHCP_HOME%\bin\yajsw-stable-11.0\bat\`)
...
*   **Linux/macOS:**
    ...
    *   **Daemon (YAJSW):**
        ```bash
        sudo ./bin/yajsw-stable-11.0/bat/queryDaemon.sh 
        # Or: sudo ./bin/yajsw-stable-11.0/bat/statusDaemon.sh
        ```
        Or, using systemd: `sudo systemctl status JagornetDhcpServer`
*   **Windows:**
    *   **Service:** Open the Services console (services.msc) and check the status of "Jagornet DHCP Server".
    *   Alternatively, from an Administrator Command Prompt:
        ```cmd
        sc query JagornetDhcpServer 
        # Note: The actual service name might vary slightly if changed during installation.
        # It's typically "JagornetDhcpServer" or "Jagornet DHCP Server".
        ```
...
*   **YAJSW Wrapper Logs (when running as a service/daemon):**
    *   Configuration: Controlled by the `wrapper.conf` file in the YAJSW directory (e.g., `$JAGORNET_DHCP_HOME/bin/yajsw-stable-11.0/conf/wrapper.conf`).
    *   Typical Location: `$JAGORNET_DHCP_HOME/logs/wrapper.log` (this path can be confirmed or changed in `wrapper.conf`).
    *   These logs are specific to the Java Service Wrapper. They will show issues related to starting or stopping the Java Virtual Machine (JVM) for the server, service installation problems, etc.
...
*   **Linux/macOS:**
    Make sure the scripts are executable (`chmod +x ./bin/dhcpserver`).
    *   **DHCPv4 Test Client:**
        ```bash
        cd $JAGORNET_DHCP_HOME
        ./bin/dhcpserver testclientv4 [options]
        # For help on options:
        ./bin/dhcpserver testclientv4 -? 
        ```
        Example: To send a discover from a specific interface:
        `./bin/dhcpserver testclientv4 -i eth0`
    *   **DHCPv6 Test Client:**
        ```bash
        cd $JAGORNET_DHCP_HOME
        ./bin/dhcpserver testclientv6 [options]
        # For help on options:
        ./bin/dhcpserver testclientv6 -?
        ```
        Example: To solicit for options on a specific interface:
        `./bin/dhcpserver testclientv6 -i eth0 -information-only SOLICIT`

*   **Windows:**
    *   **DHCPv4 Test Client:**
        ```cmd
        cd %JAGORNET_DHCP_HOME%
        bin\JagornetDhcpServer.bat testclientv4 [options]
        REM For help on options:
        bin\JagornetDhcpServer.bat testclientv4 -?
        ```
        Example:
        `bin\JagornetDhcpServer.bat testclientv4 -i "Ethernet 2"` (use interface name from `list-interfaces`)
    *   **DHCPv6 Test Client:**
        ```cmd
        cd %JAGORNET_DHCP_HOME%
        bin\JagornetDhcpServer.bat testclientv6 [options]
        REM For help on options:
        bin\JagornetDhcpServer.bat testclientv6 -?
        ```
        Example:
        `bin\JagornetDhcpServer.bat testclientv6 -i "Ethernet 2" -information-only SOLICIT`
...
Remember to consult the output of `./bin/dhcpserver list-interfaces` (or `bin\JagornetDhcpServer.bat list-interfaces` on Windows) to get the correct interface names for the `-i` option if needed by the test clients.
```
...
2.  **Review Logs (Crucial First Step):**
    ...
    *   **YAJSW Wrapper Logs:** If running as a service/daemon, also check the YAJSW logs. The typical location is `$JAGORNET_DHCP_HOME/logs/wrapper.log`. However, depending on YAJSW configuration or if the main `logs` directory hasn't been created or centralized yet, it might also be in `$JAGORNET_DHCP_HOME/bin/yajsw-stable-11.0/log/wrapper.log` (adjust `yajsw-stable-11.0` if your version is different). These logs will show errors related to the service wrapper itself, such as problems starting the JVM.

3.  **Configuration File Errors:**
    *   **Validate Syntax:** Use the `test-configfile` command to check your configuration file for syntax errors. Navigate to `$JAGORNET_DHCP_HOME` and run:
        ```bash
        ...
        bin\JagornetDhcpServer.bat test-configfile -c config\dhcpserver.xml
        ```
...
4.  **Port Conflicts:**
    ...
    *   **Privileged Ports (Linux/macOS):** Ports below 1024 are privileged. If you try to run `./bin/dhcpserver start` directly without `sudo`, it will fail to bind to these ports. Using `sudo` or running as a service (which is usually configured to run as root or with necessary capabilities) is required.

5.  **Permissions (Linux/macOS):**
    *   **Execute Permissions:** Ensure the server scripts have execute permissions (e.g., `chmod +x ./bin/dhcpserver` and the YAJSW scripts). This was covered in the "Important: Script Permissions" note earlier.
    *   **Privileged Ports:** As mentioned above, direct execution for privileged ports (like 67, 547) requires `sudo`. Service installations via YAJSW typically handle the necessary privileges.
...
2.  **Review Logs (Crucial First Step):**
    *   **Application Logs:** Check `$JAGORNET_DHCP_HOME/logs/JagornetDhcpServer.log`. Look for Java stack traces or error messages that indicate why the application itself failed to initialize or run.
    *   **YAJSW Wrapper Logs:** If running as a service/daemon, also check the YAJSW logs, typically located at `$JAGORNET_DHCP_HOME/logs/wrapper.log` (or sometimes in `$JAGORNET_DHCP_HOME/bin/yajsw-stable-11.0/log/wrapper.log` if `logs` directory hasn't been centralized by `wrapper.conf`). These logs will show errors related to the service wrapper itself, such as problems starting the JVM.
...
5.  **Permissions (Linux/macOS):**
    *   **Execute Permissions:** Ensure the server scripts have execute permissions (e.g., `chmod +x ./bin/dhcpserver`). This was covered in the "Important: Script Permissions" note earlier.
