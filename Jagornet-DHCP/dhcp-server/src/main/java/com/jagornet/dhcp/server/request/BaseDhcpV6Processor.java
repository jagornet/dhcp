...
public abstract class BaseDhcpV6Processor implements DhcpV6MessageProcessor {
	...
	public DhcpV6ServerIdOption getDhcpV6ServerIdOption() {
		return dhcpServerConfig.getDhcpV6ServerIdOption();
	}
	...
	protected void populateReplyMsgOptions(DhcpLink dhcpLink) {
		Map<Integer, DhcpOption> optionMap = dhcpServerConfig.effectiveMsgOptions(requestMsg, dhcpLink);
	...
	public DhcpV6Message processMessage() {
		try {
			...
			// ]]replyMsg = new DhcpV6Message(requestMsg.getLocalAddress(),
			...
			replyMsg.putDhcpOption(getDhcpV6ServerIdOption());
	...
	protected boolean shouldMulticast() {
		if (requestMsg.isUnicast()) {
			Map<Integer, DhcpOption> effectiveMsgOptions = dhcpServerConfig.effectiveMsgOptions(requestMsg, clientLink);
	...
	protected void processDdnsUpdates(boolean sendUpdates) {
		DhcpV6ClientFqdnOption clientFqdnOption = (DhcpV6ClientFqdnOption) requestMsg
				.getDhcpOption(DhcpConstants.V6OPTION_CLIENT_FQDN);
