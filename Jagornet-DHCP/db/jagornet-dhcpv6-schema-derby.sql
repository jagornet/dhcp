...
-- * DUID (not null) [foreign_key] - 128 byte max + 2 for code = 130 bytes
...
CREATE TABLE IAADDRESS (
    ID BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    IPADDRESS VARCHAR(16) FOR BIT DATA NOT NULL UNIQUE,
    STARTTIME TIMESTAMP,
...
CREATE UNIQUE INDEX IPADDRESS_NDX ON IAADDRESS (IPADDRESS);
...
CREATE UNIQUE INDEX PREFIX_NDX ON IAPREFIX (PREFIXADDRESS, PREFIXLENGTH);
...
CREATE TABLE DHCPOPTION (
    ID BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    CODE INTEGER,
    VALUE VARCHAR(1024) FOR BIT DATA,
    IDENTITYASSOC_ID BIGINT,
    IAADDRESS_ID BIGINT,
    IAPREFIX_ID BIGINT,
    FOREIGN KEY (IDENTITYASSOC_ID) REFERENCES IDENTITYASSOC(ID) ON DELETE CASCADE,
