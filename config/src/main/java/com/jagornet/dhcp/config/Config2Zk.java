 ...
 */
public class Config2Zk implements Watcher {

    private static final Logger log = LoggerFactory.getLogger(DhcpServerConfiguration.class);


    private Config2Zk(DhcpServerConfiguration dsc, String zkServers, String zkRoot, boolean reset) throws Exception {
    ...
    public static Config2Zk start(DhcpServerConfiguration dsc, String zkServers, String zkUrl, boolean reset) throws Exception {
        return new Config2Zk(dsc, zkServers, zkUrl, reset);
    }
    public static void cleanup(String zkServers, String zkUrl) throws Exception {
        final CuratorFramework curatorFramework = CuratorFrameworkFactory.newClient(zkServers, new ExponentialBackoffRetry(1000, 3));
        curatorFramework.start();
    ...
    public static void main(String[] argv)  {
        if (argv.length < 3) {
            ...
            try {
                ...
                dsc.init(argv[0]);
                Config2Zk.start(dsc, argv[1], argv[2], argv.length > 3);
                System.exit(0);
            } catch (Exception e) {
