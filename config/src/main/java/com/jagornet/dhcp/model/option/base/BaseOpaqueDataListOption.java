...
public abstract class BaseOpaqueDataListOption extends BaseDhcpOption implements DhcpComparableOption
{
    ...
    private final List<BaseOpaqueData> opaqueDataList = new LinkedList<>();
    ...
    public BaseOpaqueDataListOption(OpaqueDataListOptionType opaqueDataListOption)
    {
        ...
        if (opaqueDataListOption != null) {
			...
			setOpaqueDataList(opaqueDataListOption.getOpaqueDataArray());
	...
	public void setOpaqueDataList(OpaqueData[] opaqueDataList) {
		this.opaqueDataList.clear();
		for (OpaqueData bod : opaqueDataList) {
			this.opaqueDataList.add(new BaseOpaqueData(bod));
		}
	}
	...
	{
		if (that != null) {
        	OpaqueData[] opaqueList = that.getOpaqueDataArray();
        	if (opaqueList != null) {
        		if (op.equals(Operator.EQUALS)) {
	        		if (opaqueList.length != opaqueDataList.size()) {
	        			return false;
	        		}
	        		for (int i=0; i<opaqueList.length; i++) {
